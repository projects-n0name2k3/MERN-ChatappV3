...
import { IoIosArrowDown, IoIosAttach, IoIosClose, IoMdImages, IoIosArrowUp, IoIosAdd } from "react-icons/io";
    ...
    <div className="container mx-auto flex min-h-screen">
      ...
      >
        <div className="flex items-center justify-between border-b border-muted h-[84px] py-4 px-6">
          <div className="flex flex-col ">
            <p
              className={`font-lexend font-bold text-xl text-neutral-900 leading-[30px] transition-all duration-300`}
            >
              Chat Detail
            </p>
            <span className="font-manrope text-sm text-neutral-500 leading-[22px]">
        ...
        <div className="p-6 flex flex-col ">
          <div className="flex items-center justify-between">
            <p className="font-lexend text-base leading-[26px] text-neutral-900 font-semibold select-none">Members</p>
            <div className="flex items-center gap-5">
              <span  className="font-manrope text-sm text-neutral-500 leading-[22px] select-none">3 participants</span>
        {
          showParticipants ? (
            <IoIosArrowUp size={24} className="icon text-neutral-600" onClick={() => setShowParticipants(false)}/>
          ) : (
            <IoIosArrowDown size={24} className="icon text-neutral-600" onClick={() => setShowParticipants(true)}/>
          )
        }
            </div>
          </div>
          <div className={`flex flex-col mt-4 gap-4 overflow-hidden transition-all duration-500 ease-out ${showParticipants ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
  {
    showParticipants && participants.map((participant, index) => (
      <div className="flex items-center justify-between gap-3" key={index}>
       ...
       <Avatar className="cursor-pointer relative size-9">
        ...
        <p className="font-manrope text-sm text-neutral-900 leading-[22px] select-none">{participant.name}</p>
       </div>
       <div className="flex items-center gap-4">
        {
          isAdmin && (
            <span className="px-2 py-1 text-xs font-manrope text-neutral-700 select-none rounded-lg bg-yellow-400">Admin</span>
          )
        }
<BsThreeDots
            size={24}
      ...
      <Button variant={"ghost"} className={`w-1/4 font-manrope text-primary hover:text-primary/80 hover:bg-white ${showParticipants ? 'flex' : 'hidden'}`}><IoIosAdd size={24}/>Add</Button>
</div>
<span className="bg-neutral-200 h-[1px]"></span>

<div className="flex items-center justify-between mt-4">
            <p className="font-lexend text-base leading-[26px] text-neutral-900 font-semibold select-none">Gallery</p>
            <div className="flex items-center gap-5">
        {
          showGallery ? (
            <IoIosArrowUp size={24} className="icon text-neutral-600" onClick={() => setShowGallery(false)}/>
          ) : (
            <IoIosArrowDown size={24} className="icon text-neutral-600" onClick={() => setShowGallery(true)}/>
          )
        }
            </div>
          </div>
